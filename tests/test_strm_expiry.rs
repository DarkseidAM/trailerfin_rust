...
use trailerfin_rust::trailers::scraper::is_strm_expired;

#[test]
fn test_expired_strm_file_with_expired_timestamp() {
    ...
    assert!(is_strm_expired(&file_path).unwrap());
}

#[test]
fn test_valid_strm_file_with_future_expiry() {
    let dir = tempdir().unwrap();
...
#[test]
fn test_strm_file_missing_expiry() {
    let dir = tempdir().unwrap();
...
#[test]
fn test_strm_file_missing_entirely() {
    let bogus_path = PathBuf::from("totally_missing.strm");
    assert!(is_strm_expired(&bogus_path).unwrap());
}